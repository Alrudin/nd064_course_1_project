...
# added a counter to track the number of connections made 'db_connection_count'
...
def post(post_id):
    ...
    if post is None:
      logger.error('Article with id %s not found', post_id)
...
def create():
    if request.method == 'POST':
        ...
        if not title:
            ...
            connection.commit()
            connection.close()
            logger.info('A new article "%s" created!', title)
